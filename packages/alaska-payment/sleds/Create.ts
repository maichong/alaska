...
export default class Create extends Sled<CreateParams, Payment> {
  async exec(params: CreateParams): Promise<Payment> {
    ...
    if (!paymentService.paymentPlugins.has(payment.type)) paymentService.error('Unknown payment type');
    ...
    payment.params = await paymentService.paymentPlugins.get(payment.type).createParams(payment);
    await payment.save({ session: this.dbSession });
    return payment;
  }
}
